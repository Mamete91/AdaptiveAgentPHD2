...
agente 297, plugin 26, build 0 warning. §59: chiusura NINA istantanea (delega dopo il 200; watchdog di auto-terminazione 25s nell agente rende il 200 un contratto). §60: plugin localizzato via resx embedded (Follow N.I.N.A./EN/IT, cambio live, 80 chiavi incl. 6 tooltip parametri N6, log sempre EN) — chiusura certificata da audit (0 residui non classificati) e lifecycle (auto-launch + manage external) ON di default con kill-switch. §76: il canale guida (3 s) può ora anticipare l'unsafe quando il cielo peggiora prima che N1 (300 s) se ne accorga — mai il contrario; §76-bis: cricchetto sul riferimento SNR (rana bollita del §66 ritrovata identica, risana anche il §57). §77: pannello "Condizioni del Cielo" che racconta le due voci. §75: rimosso il toggle AI Finder (un interruttore che, acceso, scavalcava il backoff §17 nato da un incidente reale; il suo unico valore — la saturazione — è oggi nativo via §68). Resta star_finder.py, usato dal Path B. La selezione stella è competenza di PHD2. §74: pannelli dinamici — i sottosistemi del motore (Controller, Esposizione, Escalation Gate, Adaptive MinMove) occupano spazio in proporzione all'attività reale: barra compatta da fermi, apertura automatica quando intervengono, chiusura dopo 120s ma mai contro una scelta dell'operatore. §73: stato del Safety Monitor nella dashboard — nuovo canale plugin→agente (POST /nina/safety) e chip di stato al posto del toggle MODALITÀ TEST; stati veri (SAFE/UNSAFE+causa/MERIDIAN_PROTECTION), stantio = UNKNOWN mai SAFE. §71: gate della sonda sul "canale pronto" (consenso AND §68, deferrer con tetto 15 min, S2 prioritario, fail-open). §72: MERIDIAN_PROTECTION — finestra limitata di Safe riportato per la sola manovra di flip in unsafe prolungato (anti-deadlock 19/7; latch interni intatti; lockout anti-oscillazione; riattivazione tracking ex-post). §70: ereditarietà del contesto in N1 — le immagini senza target (le sonde: vivono fuori dal contenitore target) ereditano l'ultimo target dichiarato invece di aprire una chiave vergine auto-referenziale (falso SAFE del 3/8: sonda 7/7=1.00). §69: pedaggio IPv6 di `localhost` rimosso su ENTRAMBI i collegamenti — plugin→agente (POST /shutdown 2031→5 ms, chiusura NINA 3043→402 ms) e agente→PHD2 (PHD2 ascolta solo IPv4: wxIPV4address) + deduplica del log di retry (85% del file → poche righe, con battito e sintesi). §68: osservabilità del canale di guida — telemetria PHD2 mai usata (LoopingExposures, ErrorCode, Alert.Type, StarMass) + quinto latch GUIDE_UNOBSERVABLE (leaky, corroborazione che accelera ma non decide); chiude il guasto fail-dangerous del 26/7. §67: baseline N1 per (target, filtro) — il cambio campo lo dichiara NINA invece di dedurlo; best di sessione promosso a pavimento del rilascio; airmass in telemetria (nessuna decisione). §66: cricchetto del riferimento N1 (miglioramento adottato subito, congelato durante l'evento con tetto, rilascio lento a cielo sereno) — fine della "rana bollita" osservata sul cielo; high-water di sessione esposto come diagnostico. §65: rientro dal latch STAR_LOST su guida OPERATIVA (non su guida eccellente). §64: cadenza della Recovery probe DERIVATA — v2: timeout = min(finestra §43 − posa, 3 min validati sul cielo); finestra letta da /status, non duplicata — parametro manuale retrocesso a escape hatch dietro "Auto" (default ON). §63: primo bug di campo v2.8 (NameError wiring §57 → loop-crash ciclico, GUARDIAN inerte) fixato in v2.8.1 con test sul VERO _event_loop + riga "Ciclo motore" tri-stato in dashboard; caso N6-non-SAFE aperto, da analizzare coi log NINA. §62 REGOLA PERMANENTE: rebuild del distribuibile (ZIP/DLL installata) = parte della Definition of Done di ogni modifica al codice, anche senza commit. §61: proprietà verificata sui sorgenti NINA — il Sequence Engine resta l'unica autorità (la fine sequenza cancella la Recovery probe via catena di token linked, anche a metà posa; il SAFE post-sequenza è inerte). §57-ter: 2ª limitazione GUI (il trigger rifiuta anche le condizioni di ciclo) → il ciclo di recovery è INTERNO alla Recovery probe: setup = la sola istruzione nel Before. Dettaglio: NOTE_CLAUDE §57-ter + §58 + §58-bis.
