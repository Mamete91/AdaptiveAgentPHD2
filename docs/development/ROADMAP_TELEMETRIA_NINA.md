...
Entrambe le metà fatte: l'Agente riceve (§41) e il plugin inoltra (§42). Il tubo è aperto end-to-end; **nessun consumatore agisce ancora** sui dati. Sbloccati N1–N8.

- [x] **Lato Agente — FATTO (§41, 2026-06-18).** Canale in ingresso `POST /nina/telemetry` (endpoint difensivo, pydantic `NinaTelemetryPayload`), store opzionale/thread-safe `NinaTelemetryStore` (Layer-1 grezzo: ultimo valore + storico + `is_fresh`, nessuna logica derivata), blocco top-level `nina` in `/status`, config `[nina_telemetry] enabled=true` (born-operative, kill-switch). Contratto JSON **`schema_version=1`** fissato. Opzionale e graceful: senza POST l'Agente è bit-identico; payload malformato → 422 senza mai disturbare la guida. **Nessun consumatore agisce** (N1–N8 successivi). 203 test verdi. Dettagli: NOTE_CLAUDE §41.
- [x] **Lato plugin — FATTO (§42, 2026-06-18).** `AgentTelemetryForwarder` (repo plugin) iscritto a `IImageSaveMediator.ImageSaved`; a ogni light salvata POSTa `POST <DashboardUrl>/nina/telemetry` (contratto `schema_version=1`, solo `image{}`): HFR, HFR std, conteggio stelle, statistiche ADU (mean/median/stdev), durata, filtro. **Opzionale e graceful** (handler non-throwing, POST fire-and-forget, timeout 3 s, swallow totale: Agente offline → no-op, NINA mai disturbata), toggle `ForwardTelemetryToAgent` default ON. Plugin **v1.3.0.0**, build Release 0 errori/0 warning contro NINA **3.2.0.9001**. **Lezione verify-before-implement:** la tabella API era dal ramo `develop`, ma `FWHM`/`Eccentricity` **non** esistono su `IStarDetectionAnalysis` in 3.2.0.9001 (aggiunti dopo) → omessi finché la NINA installata non li espone; il contratto §41 (campo `fwhm`) resta forward-ready. Congelamento plugin rimosso (Alessandro 2026-06-18): si sviluppa/builda/valida sul PC corrente (.NET 8 SDK installato). Dettagli: NOTE_CLAUDE §42.
